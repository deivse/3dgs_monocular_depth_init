...
def create_splats_with_optimizers(
    ...
    params = [
        ("means", torch.nn.Parameter(points), 0.00016 * scene_scale),
        ("scales", torch.nn.Parameter(scales), 0.005),
        ("quats", torch.nn.Parameter(quats), 0.001),
        ("opacities", torch.nn.Parameter(opacities), 0.05),
    ]
...
class Runner:
    ...
    def __init__(
        self, local_rank: int, world_rank, world_size: int, cfg: Config, Parser, Dataset
    ) -> None:
        ...
        if cfg.use_bilateral_grid:
            ...
            self.bil_grid_optimizers = [
                torch.optim.Adam(
                    ...
                    lr=0.002 * math.sqrt(cfg.batch_size),
                    eps=1e-15,
                )
            ]
    ...
    def eval(self, step: int, stage: str = "val"):
        ...
        if world_rank == 0:
            ...
            print(
                f"PSNR: {stats['psnr']:.3f}, SSIM: {stats['ssim']:.4f}, LPIPS: {stats['lpips']:.3f} Time: {stats['ellipse_time']:.3f}s/image Number of GS: {stats['num_GS']}"
            )
    ...
    def render_traj(self, step: int):
        ...
        if cfg.render_traj_path == "interp":
            cam_to_worlds_all = generate_interpolated_path(cam_to_worlds_all, 1)
    ...
    def train_iteration(self, step):
        ...
        try:
            ...
            trainloader_iter = iter(trainloader)
            data = next(trainloader_iter)
        cam_to_worlds = cam_to_worlds_gt = data["camtoworld"].to(device)
        Ks = data["K"].to(device)
        pixels = data["image"].to(device) / 255.0
        ...
        if not cfg.random_background and cfg.background_color is not None:
            bkgd = torch.tensor(
                cfg.background_color, dtype=colors.dtype, device=self.device
            ).view(1, 1, 1, 3)
        ...
        if device.startswith("cuda"):
            ...
            allocated_str = "-"
        desc = f"Loss: {loss.item():.3f} | SH deg.: {sh_degree_to_use} | CUDA Alloc.: {allocated_str}"
        if cfg.depth_loss:
