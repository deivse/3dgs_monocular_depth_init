...
import shutil
import glob
from metric3d.mono.utils.running import main_process
import torch
...
def save_raw_imgs(
    ...
    cv2.imwrite(
        os.path.join(save_dir, filename[:-4] + "_d.png"),
...
def save_val_imgs(
    ...
    """
    rgb, pred_scale, target_scale, pred_color, target_color = get_data_for_log(
        pred, target, rgb
    )
    ...
    if tb_logger is not None:
        tb_logger.add_image(
            f"{filename[:-4]}_merge.jpg", cat_img.transpose((2, 0, 1)), iter
        )
