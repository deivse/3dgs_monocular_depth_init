...
import random
import numpy as np
from metric3d.mono.utils.logger import setup_logger
import glob
from metric3d.mono.utils.comm import init_env
...
from metric3d.mono.utils.do_test import do_scalecano_test_with_custom_data
from metric3d.mono.utils.mldb import load_data_info, reset_ckpt_path
from metric3d.mono.utils.custom_data import load_from_annos, load_data
...
def parse_args():
    ...
    parser.add_argument(
        "--show-dir", help="the dir to save logs and visualization results"
    )
    ...
    parser.add_argument(
        "--test_data_path", default="None", type=str, help="the path of test data"
    )
...
def main(args):
    ...
    if args.show_dir is not None:
        ...
        cfg.show_dir = osp.join(
            "./show_dirs", osp.splitext(osp.basename(args.config))[0], args.timestamp
        )
    ...
    if not cfg.distributed:
        ...
        if args.launcher == "ror":
            ...
            mp.spawn(
                main_worker,
                nprocs=cfg.dist_params.num_gpus_per_node,
                args=(cfg, args.launcher, test_data),
            )


def main_worker(local_rank: int, cfg: dict, launcher: str, test_data: list):
    if cfg.distributed:
        cfg.dist_params.global_rank = (
            cfg.dist_params.node_rank * cfg.dist_params.num_gpus_per_node + local_rank
        )
    ...
    do_scalecano_test_with_custom_data(
        model, cfg, test_data, logger, cfg.distributed, local_rank
    )
