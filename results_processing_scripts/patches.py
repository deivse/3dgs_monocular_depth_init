...
def accumulate_param_bins(patches_a, patches_b, num_sfm_points, bin_size):
    ...
    return {bin_id: sum / count for bin_id, (sum, count) in bins_sum_count.items()}, {bin_id: count for bin_id, (_, count) in bins_sum_count.items()}
...
class MakeTableFuncs:
    ...
    @staticmethod
    def patches_binned_improvement_dataset_avg(data_loader: DataLoaderPatches, args) -> list[Table]:
        preset_a, preset_b = get_presets(args, data_loader)
        print(
            f"Computing improvement of preset B '{preset_b}' over preset A '{preset_a}'")
        ...
        def make_param_bins(param: Parameter):
            ...
            for scene in data_loader.scenes:
                try:
                    patches_per_image_a = data_loader.get(
                        scene, preset_a, param)
                    patches_per_image_b = data_loader.get(
                        scene, preset_b, param)
                except KeyError:
                    raise RuntimeError(
                ...
                patches_a.extend(
                    p for patches in patches_per_image_a for p in patches)
                patches_b.extend(
                    p for patches in patches_per_image_b for p in patches)
        ...
        for param in data_loader.params:
            ...
            if min_bin_id is None or max_bin_id is None:
                min_bin_id, max_bin_id = min(
                    param_bins.keys()), max(param_bins.keys())
            else:
            ...
            for bin_id, value in param_bins.items():
                ...
                combined_bins[bin_id]["sample_count"] = \
                    combined_bins[bin_id].get(
                        "sample_count", 0) + bin_counts[bin_id]

        header_row = [f"{args.dataset}"] + [make_bin_name(args.bin_size, i)
                                            for i in range(min_bin_id, max_bin_id+1)]
        table = [header_row]
        for param in data_loader.params:
            if param.name == PARAMS["num_sfm_points"].name:
            ...
            for bin_id in range(min_bin_id, max_bin_id + 1):
                if bin_id in combined_bins and param.name in combined_bins[bin_id]:
                    row.append(param.make_instance(
                        combined_bins[bin_id][param.name]).get_formatted_value())
                else:
                    row.append("-")
            table.append(row)
        ...
        for bin_id in range(min_bin_id, max_bin_id + 1):
            bin_row.append(
                f"{combined_bins[bin_id]['sample_count']}" if bin_id in combined_bins else "0")
        table.append(bin_row)

        return [table]

    @staticmethod
    def patches_improvement_percentile(data_loader: DataLoaderPatches, args) -> list[Table]:
        ...
        return [table_for_param(param) for param in data_loader.params if param.name != PARAMS["num_sfm_points"].name]

    @staticmethod
    def patches_improvement_percentile_dataset_avg(data_loader: DataLoaderPatches, args) -> list[Table]:
        preset_a, preset_b = get_presets(args, data_loader)
        print(
            f"Computing improvement of preset B '{preset_b}' over preset A '{preset_a}'")
        ...
        def table_for_param(param: Parameter):
            percentile_improvements = [
                np.empty([0]) for _ in range(len(percentiles))
            ]
            for scene in data_loader.scenes:
                try:
                    patches_per_image_a = data_loader.get(
                ...
                m = -1 if (param.ordering.value ==
                           ParamOrdering.LOWER_IS_BETTER.value) else 1
                percentile_indices = patch_percentile_indices(
                    patch_vals_a * m, percentiles)
                for i, indices in enumerate(percentile_indices):
                    improvements = patch_vals_b[indices] - \
                        patch_vals_a[indices]
...
def main():
    ...
    patches_binned_impr_da.add_argument(
        "--bin-size", type=int, default=100, help="Bin size for binning num sfm points")
    patches_binned_impr_da.add_argument("--preset-regex-a", type=str, default="init-type=sfm",
                                        help="Regex for the first preset, improvement for preset B will be computed relative to this. Must select single preset for each scene.")
    patches_binned_impr_da.add_argument("--preset-exclude-a", type=str, default=None,
                                        help="Regex for the first preset, improvement for preset B will be computed relative to this. Must select single preset for each scene.")
    patches_binned_impr_da.add_argument("--preset-regex-b", type=str, default=None,
                                        help="Regex for the second preset, improvement for this preset will be computed relative to preset A. Must select single preset for each scene.")
    patches_binned_impr_da.add_argument("--preset-exclude-b", type=str, default=None,
                                        help="Regex for the second preset, improvement for this preset will be computed relative to preset A. Must select single preset for each scene.")
    patches_binned_impr_da.add_argument(
        "params", nargs="+", choices=PARAMS.keys())
    ...
    patches_impr_perc_da.add_argument("--preset-regex-a", type=str, default="init-type=sfm",
                                      help="Regex for the first preset, improvement for preset B will be computed relative to this. Must select single preset for each scene.")
    patches_impr_perc_da.add_argument("--preset-exclude-a", type=str, default=None,
                                      help="Regex for the first preset, improvement for preset B will be computed relative to this. Must select single preset for each scene.")
    patches_impr_perc_da.add_argument("--preset-regex-b", type=str, default=None,
                                      help="Regex for the second preset, improvement for this preset will be computed relative to preset A. Must select single preset for each scene.")
    patches_impr_perc_da.add_argument("--preset-exclude-b", type=str, default=None,
                                      help="Regex for the second preset, improvement for this preset will be computed relative to preset A. Must select single preset for each scene.")
