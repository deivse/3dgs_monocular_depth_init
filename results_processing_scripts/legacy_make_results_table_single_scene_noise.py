...
def main():
    ...
    for preset_dir in scene_dir.iterdir():
        if not preset_dir.is_dir():
            continue
        if "noise" not in preset_dir.name or "depth_downsample_40" in preset_dir.name:
            continue
        noise_preset = preset_dir.name[: preset_dir.name.index("noise") - 1]
        param_vals = get_params(preset_dir, args.step, param_to_param_path)
        results_by_preset[preset_dir.name] = param_vals

    assert noise_preset is not None

    for preset in [noise_preset]:
        preset_dir = scene_dir / preset
        assert preset_dir.is_dir(), f"Directory {preset} not found in {scene_dir}"
