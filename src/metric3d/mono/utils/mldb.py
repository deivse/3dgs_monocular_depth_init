...
def load_data_info(module_name, data_info={}, mldb_type="mldb_info", module=None):
    ...
    if module:
        ...
        raise RuntimeError(
            f'Try to access "mldb_info", but cannot find {module_name} module.'
        )
...
def reset_ckpt_path(cfg, data_info):
    if isinstance(cfg, dict):
        for key in cfg.keys():
            if key == "backbone":
                new_ckpt_path = (
                    data_info["checkpoint"]["mldb_root"]
                    + "/"
                    + data_info["checkpoint"][cfg.backbone.type]
                )
