...
def get_prediction(
    ...
    confidence = confidence[
        pad_info[0] : confidence.shape[0] - pad_info[1],
        pad_info[2] : confidence.shape[1] - pad_info[3],
    ]
...
def do_scalecano_test_with_custom_data(
    ...
    dam_median = MetricAverageMeter(
        ["abs_rel", "rmse", "silog", "delta1", "delta2", "delta3"]
    )
    dam_global = MetricAverageMeter(
        ["abs_rel", "rmse", "silog", "delta1", "delta2", "delta3"]
    )
    ...
    for i, an in tqdm(enumerate(test_data)):
        ...
        if gt_depth_flag:
            pred_depth = torch.nn.functional.interpolate(
                ...
                (gt_depth.shape[0], gt_depth.shape[1]),
                mode="bilinear",
            ).squeeze()  # to original size
            ...
            pred_depth_median = (
                pred_depth
                * gt_depth[gt_depth != 0].median()
                / pred_depth[gt_depth != 0].median()
            )
            pred_global, _ = align_scale_shift(pred_depth, gt_depth)

            mask = gt_depth > 1e-8
            dam.update_metrics_gpu(pred_depth, gt_depth, mask, is_distributed)
            dam_median.update_metrics_gpu(
                pred_depth_median, gt_depth, mask, is_distributed
            )
            dam_global.update_metrics_gpu(pred_global, gt_depth, mask, is_distributed)
            print(
                gt_depth[gt_depth != 0].median() / pred_depth[gt_depth != 0].median(),
            )

        if i % save_interval == 0:
            os.makedirs(osp.join(save_imgs_dir, an["folder"]), exist_ok=True)
            rgb_torch = (
                torch.from_numpy(rgb_origin).to(pred_depth.device).permute(2, 0, 1)
            )
            mean = (
            ...
            save_val_imgs(
                ...
                gt_depth
                if gt_depth is not None
                else torch.ones_like(pred_depth, device=pred_depth.device),
                rgb_torch,
            ...
            if an["intrinsic"] == None:
                ...
                for r in [1.0]:
                    ...
                    for f in [1000]:
                        pcd = reconstruct_pcd(
                            pred_depth, f * r, f * (2 - r), intrinsic[2], intrinsic[3]
                        )
                        fstr = "_fx_" + str(int(f * r)) + "_fy_" + str(int(f * (2 - r)))
                        os.makedirs(osp.join(save_pcd_dir, an["folder"]), exist_ok=True)
                        save_point_cloud(
                            pcd.reshape((-1, 3)),
                            rgb_origin.reshape(-1, 3),
                            osp.join(
                                save_pcd_dir,
                                an["folder"],
                                an["filename"][:-4] + fstr + ".ply",
                            ),
                        )
        ...
        if "normal_out_list" in output.keys():
            ...
            if False:
                ...
                dam.update_normal_metrics_gpu(
                    pred_normal, gt_normal, gt_normal_mask, cfg.distributed
                )  # save valiad normal
            ...
            if i % save_interval == 0:
                save_normal_val_imgs(
                    ...
                    else torch.ones_like(pred_normal, device=pred_normal.device),
